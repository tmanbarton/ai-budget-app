...
public class ClaudeBudgetParser
{
    ...
    public static async Task<BudgetEntry> ParseBudgetEntryAsync(string userInput)
    {
        ...
        var requestBody = new
        {
            ...
            - If the user's description of the transaction includes a store name whose products can be in multiple categories (e.g. Home Depot sells both hoome improvement products - "Housing" - and tools - "Other" or "Entertainment"), you should ask the user to clarify which category it belongs to, e.g. User: "I spent $100 at Home Depot last week" You should respond with "NeedsClarification": "Does the transaction belong in Housing, Entertainment, or Other category?" and leave the category field null.
            - If the user doesn't provide a specific date/relative day (e.g. "yesterday"), assume the transaction happened today. Exception: if the user says something like "I spent $20 on a movie last week", you should clarify what the day was since it is ambiguous. For this case, you would set NeedsClarification to "What day did the transaction happen?" and leave the date field null.
            - If the user doesn't provide a specific amount, you should set NeedsClarification to "How much was the transaction for?" and leave the amount field null.
            - If the transaction clearly fits into one category and has a clear amount and date, set NeedsClarification to null.
